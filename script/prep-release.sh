...
make bin/ipxe.dsk bin/ipxe.iso bin/ipxe.lkrn bin/ipxe.usb bin/ipxe.kpxe bin/undionly.kpxe EMBED=../../ipxe/disks/netboot
# TRUST=ca-ipxe-org.crt,ca-netboot-xyz.crt
mv bin/ipxe.dsk ../../build/ipxe/netboot.dsk
mv bin/ipxe.iso ../../build/ipxe/netboot.iso
mv bin/ipxe.lkrn ../../build/ipxe/netboot.lkrn
mv bin/ipxe.usb ../../build/ipxe/netboot.usb
mv bin/ipxe.kpxe ../../build/ipxe/netboot.kpxe
mv bin/undionly.kpxe ../../build/ipxe/netboot-undionly.kpxe

# generate netboot.xyz iPXE disk for Google Compute Engine
make bin/ipxe.usb CONFIG=cloud EMBED=../../ipxe/disks/netboot-gce
# \TRUST=ca-ipxe-org.crt,ca-netboot-xyz.crt
cp -f bin/ipxe.usb disk.raw
tar Sczvf netboot-gce.tar.gz disk.raw
mv netboot-gce.tar.gz ../../build/ipxe/netboot-gce.tar.gz

# generate netboot.xyz-packet iPXE disk
make bin/undionly.kpxe \
EMBED=../../ipxe/disks/netboot-packet
# TRUST=ca-ipxe-org.crt,ca-netboot-xyz.crt
mv bin/undionly.kpxe ../../build/ipxe/netboot-packet.kpxe

# generate EFI iPXE disks
cp config/local/general.h.efi config/local/general.h
make clean
make bin-x86_64-efi/ipxe.efi EMBED=../../ipxe/disks/netboot
# TRUST=ca-ipxe-org.crt,ca-netboot-xyz.crt
mkdir -p efi_tmp/EFI/BOOT/
cp bin-x86_64-efi/ipxe.efi efi_tmp/EFI/BOOT/bootx64.efi
genisoimage -o ipxe.eiso efi_tmp
mv bin-x86_64-efi/ipxe.efi ../../build/ipxe/netboot.efi
mv ipxe.eiso ../../build/ipxe/netboot-efi.iso

# generate EFI arm64 iPXE disk
make clean
make CROSS_COMPILE=aarch64-linux-gnu- ARCH=arm64 EMBED=../../ipxe/disks/netboot bin-arm64-efi/snp.efi
mv bin-arm64-efi/snp.efi ../../build/ipxe/netboot-arm64.efi

# generate netboot.xyz-packet-arm64 iPXE disk
make clean
make CROSS_COMPILE=aarch64-linux-gnu- ARCH=arm64 EMBED=../../ipxe/disks/netboot-packet bin-arm64-efi/snp.efi
mv bin-arm64-efi/snp.efi ../../build/ipxe/netboot-packet-arm64.efi
...
make clean
make CROSS_COMPILE=aarch64-linux-gnu- ARCH=arm64 EMBED=../../ipxe/disks/netboot bin-arm64-efi/snp.efi
mv bin-arm64-efi/snp.efi ../../build/ipxe/netboot-arm64-experimental.efi
...
#cd build/
#CURRENT_TIME=`date`
#cat > netboot-sha256-checksums.txt <<EOF
...
#for ipxe_disk in `ls .`
#do
#  sha256sum $ipxe_disk >> ../netboot-sha256-checksums.txt
#done
#cat ../netboot-sha256-checksums.txt
#mv ../netboot-sha256-checksums.txt .
#cd ../..
